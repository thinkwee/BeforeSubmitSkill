...
- 🔢 **正文数字是否匹配表格**：正文和摘要中的数字 claim 会与实际 `tabular` 单元格交叉核对，并考虑四舍五入、子集、指标容差，避免误报；关键 headline 数字错误会升级。
...
检查会以**并行 agent 团队**运行（参考文献 · LaTeX 与写作 · 会议合规 · 内部一致性 · 编译），每个 agent 生成自己的报告片段，最后合并。结果写入 **`before-submit-report.md`**，按 🔴 desk-reject 风险、🟠 审稿人会皱眉、🔵 可选润色 分组；也可以渲染为最小、自包含的 **HTML** 视图。
